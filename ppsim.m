%simulate probe-probe 
function tpp = ppsim(l)
tpp = [];
tpectPP = [];
    for x = 10:10:l
    %for x = l
        for i = 1:10000
            pp = checkpp(x,4); %parameter 1 = DNA length, 2 = probe-probe interaction legnth, change line 55 too
            tpp = [tpp pp];
        end
        npp = sum(tpp);
        pectPP = npp./10000;
    end
   % tpectPP = [tpectPP pectPP];
end
%% function get check if probe-probe happens
function [PP] = checkpp(N, Lpp)
    %generate random 20 base DNA
    SET = char(['A' 'T' 'G' 'C']) ;
    %NSET = length(SET) ;
    %N = 20 ; % pick N numbers
    ...
    %break to bases with lengthes of Lpp
     a2 = size(R);
     B1 = [];
     for x = 1:a2(1,2)-Lpp+1
         a3 = R(x:x+Lpp-1);
         B1 = [B1; a3];
     end

    %4 = number of base pairs interaction (probe-probe)
    %P = findpp(Lpp);
    %Pname = uigetfile('Choose PP sequence file.');
    libP = load('Tpp.mat');
    P = libP.p4; %p4 = 4 bp pair, p6 = 6 bps, p8 = 8bps
    psize = size(P);

    %get reverse 4 base of probe DNA 
    C = '';
    for x = 1:a2(1,2)-Lpp+1
        c1 = seqrcomplement(B1(1,:));
        C = [C; c1];
    end

    %compare B sequence with P, C
    %B vs P
    Count = [];
    tCount = [];
    for x = 1:psize(1,1);
        pt = P(x,:);
        for i = 1:a2(1,2)-Lpp+1  
            if B1(i,:) == pt;
               count = 1;
            else
               count = 0;
    ...
    for x = 1:a2(1,2)-Lpp+1;
        ...
        totalCount = sum(rCount);
        trCount = [trCount totalCount];
    end

    totalCount = sum(tCount);%+sum(trCount); 
    if totalCount >= 1;
        PP = 1;
    else 
        PP = 0;
...
function n = getrand(N)
n = [];
    for x = 1:N
        r = rand;
        % 40%AT 60%GC prob = [0.2, 0.2, 0.3, 0.3];
        prob = [0.25, 0.25, 0.25, 0.25];
        rr = sum(r >= cumsum([0, prob]));
        n = [n rr];
    end 
end

%% find pp
%generate probe-probe sequences by self complementary
function tSpp = findpp(Lpp)
N1 = [];
Npp = [];
SET = char(['A' 'T' 'G' 'C']) ;
N1 = permn([1, 2, 3, 4],Lpp);
Npp = SET(N1);

%check if DNA is complement with rDNA
Nppsize = size(Npp);
rNpp = [];
for x = 1:Nppsize(1,1)
    rnpp = seqrcomplement(Npp(x,:));
    rNpp = [rNpp; rnpp];
end
checklog = logical(ones(1,Lpp));
checkpp = Npp == rNpp;
...
%generate all possible ATGC combination 
